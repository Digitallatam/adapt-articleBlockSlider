define([
	...
	var BlockSliderView = {
		...
		_blockSliderSetupEventListeners: function() {
			...
			this.listenTo(Adapt, "blockSlider:next", this._blockSliderMoveRight);
			this.listenTo(Adapt, "blockSlider:getBlock", this._getCurrentBlock);
		},
		...
		_onBlockSliderClick: function(event) { //AQUI HAY QUE METER EL AUDIO
		...
		_blockSliderMoveIndex: function(index, animate) {
			...
			//Run component's animation
			if(document.getElementById("block"+index)) this._activateAnimation(index);
		...
		_activateAnimation: function(index){
			var nameId = "block"+index;
			var element = document.getElementById(nameId);
			var attr = element.getAttribute("class");
			if(attr != "block-inner animate"){
				attr = attr + " animate";
